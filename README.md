...
### 12. Learn from history (`/zing-learn`)
Zing scans your repo's last 50 merged PRs for Sentry Seer comments developers accepted, and your last 100 closed Linear bug tickets with linked PR fixes. A clustering agent groups the findings into recurring defect classes — the same bug shape that bit you in three different PRs becomes a rule. Output is written to `.zing-learned-rules.md` at the repo root.

The audit commands (`/zing-pr-audit`, `/zing-build-audit`, `/zing-custom-audit`) read that file before they launch and inject its contents verbatim into every review agent's prompt, so the next review catches the same class of issue before it slips through again.

#### `.zing-learned-rules.md` format

The file is loaded as-is, with no schema validation, so hand-authored rules work too. To stay compatible with `/zing-learn` re-runs and with future tooling, follow the canonical template:
...
{code snippet from an actual issue}
```

```{lang}
// GOOD — {why this is correct}
{code snippet from the actual fix}
```
...
- `**Severity:**` uses the same scale as `_shared/review-core.md`: `critical`, `high`, `medium`, `low`. Severity drives the REQUEST_CHANGES/COMMENT/APPROVE recommendation at the end of a review.
- `**Source:**` is `seer-comment`, `bug-fix`, or `both`, followed by the occurrence count in parens. Hand-authored rules use `bug-fix` by convention.
- Rules are separated by `---` horizontal rules.
- The `**Examples:**` block is optional — omit it if there's no representative snippet.
- All review agents receive the full file. Each agent applies the rules relevant to its category using its own judgment — do not pre-filter.

**Caveat:** `/zing-learn` always overwrites this file on re-run. If you've hand-edited it, back up first or merge by hand.
...
- `/zing:learn` — Mine repo history for recurring defect classes and write `.zing-learned-rules.md`
