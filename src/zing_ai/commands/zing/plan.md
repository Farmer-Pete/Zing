...
1. **Fetch the live schema.** Use `WebFetch` to GET `http://localhost:{port}/viz/schema.json` from the running Zing server. Resolve `{port}` from the session URL stored in the zing file's frontmatter (extract via simple string parse), or default to `9876`. Read every property's `description` field — they carry cross-reference rules that aren't structurally enforceable (e.g. "from_node MUST resolve to a node id in the matching step").

2. **Read the shared viz-authoring reference at `~/.claude/commands/zing/_shared/viz-authoring.md`** using the Read tool. It covers side values (`unchanged` / `added` / `removed` / `diverged`) at step / node / struct / field levels, logic shape mapping, struct + kind discriminator, cross-flow kinds, a worked example, and the coverage check. The schema is the contract; viz-authoring is the how-to.

3. **Build the viz.** One step per coherent region of the system the plan touches (a module, a request path, a job pipeline, a schema-and-its-consumers). Follow viz-authoring for everything else.

4. **Run the coverage check** against the **action plan section** of the markdown you just wrote — every numbered plan step should appear as a `steps[]` entry; every new data model / function / class / module / decision point / boundary / behavioural change mentioned in the plan should appear as a node or a struct field. If the plan says "add X, remove Y, change Z's behaviour" and you can't point at X / Y / Z in the viz, the viz is incomplete.

5. **Write** to `.zing/<plan-slug>.viz.json` (sibling to `.zing/<plan-slug>.md`). The viz file's existence is what marks something as a plan — don't try to discriminate plans from non-plans by content; every plan output by `/zing/plan` is graphable by definition.
</step>
