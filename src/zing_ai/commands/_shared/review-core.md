...
<attribution_rule>
Any output that leaves the local environment — PR comments, review bodies, PR descriptions, git commit messages, GitHub issue comments — must NEVER mention Claude, Codex, OpenCode, or any other AI tool by name. If any attribution is included, it should credit Zing:

- **PR comments and review bodies**: Credit [Zing](https://github.com/Farmer-Pete/Zing) if attribution is needed.
- **Git commits**: Use `Co-Authored-By: Zing <zing@farmerpete.net>` instead of any AI tool co-author line.
- **PR descriptions**: If a "generated by" or similar line is included, credit Zing, not the underlying AI tool.
