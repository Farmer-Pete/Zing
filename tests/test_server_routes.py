...
class TestFlowAdvance(_FlowTestBase):
    """Tests for POST /command-center/flow/advance."""

    def test_advance_next_walks_queue(self) -> None:
        """Pressing next twice walks through three items in order."""
        from zing_ai.server.flow import FlowCursor

        step_id_1 = self._make_findings_session("sess-1", "Session 1")
        step_id_2 = self._make_findings_session("sess-2", "Session 2")
        step_id_3 = self._make_findings_session("sess-3", "Session 3")

        # Start at first item.
        self.fastapi_app.state.flow_cursor = FlowCursor(session_id="sess-1", step_id=step_id_1)

        with self.client.stream(
            "POST", "/command-center/flow/advance", json={"direction": "next"}
        ) as resp:
            self.assertEqual(resp.status_code, 200)

        cursor = self.fastapi_app.state.flow_cursor
        self.assertEqual(cursor.session_id, "sess-2")
        self.assertEqual(cursor.step_id, step_id_2)
        ...
        cursor = self.fastapi_app.state.flow_cursor
        self.assertEqual(cursor.session_id, "sess-3")
        self.assertEqual(cursor.step_id, step_id_3)

    def test_advance_wraps_at_end(self) -> None:
        """Pressing next from the last item wraps to the first item."""
        from zing_ai.server.flow import FlowCursor

        self._make_findings_session("wrap-1", "Wrap 1")
        step_id_2 = self._make_findings_session("wrap-2", "Wrap 2")

        # Set cursor to last item.
        self.fastapi_app.state.flow_cursor = FlowCursor(session_id="wrap-2", step_id=step_id_2)
        ...
        cursor = self.fastapi_app.state.flow_cursor
        self.assertEqual(cursor.session_id, "wrap-1")

    def test_advance_filters_unpinned_attach_from_queue(self) -> None:
        """Advancing away from an unpinned attach item removes it from the filtered queue
        so the cursor moves to the other item, but the notification is NOT marked answered."""
        from zing_ai.server.flow import FlowCursor

        # One findings item + one unpinned attach item.
        self._make_findings_session("findings-a", "Findings A")
        self._make_attach_session("attach-u", pinned=False)

        # Point cursor at the attach item.
        self.fastapi_app.state.flow_cursor = FlowCursor(session_id="attach-u", step_id=None)
        ...
        cursor = self.fastapi_app.state.flow_cursor
        # Cursor must have moved off the attach session.
        self.assertNotEqual(cursor.session_id, "attach-u")

        # Notification was NOT marked answered — auto_dismiss only filters the queue view.
        attach_session = self.manager.get_session("attach-u")
        from zing_ai.server.models import ClaudeCodeSession

        assert isinstance(attach_session, ClaudeCodeSession)
        self.assertIsNotNone(attach_session.pending_question)
        self.assertIsNone(attach_session.pending_question.answered_at)  # type: ignore[union-attr]

    def test_advance_keeps_pinned_attach(self) -> None:
        """Advancing away from a pinned attach item leaves it in the full queue."""
        from datetime import UTC, datetime

        from zing_ai.server.attention import build_attention_queue
        from zing_ai.server.flow import FlowCursor

        self._make_findings_session("findings-b", "Findings B")
        self._make_attach_session("attach-p", pinned=True)

        # Point cursor at pinned attach item.
        self.fastapi_app.state.flow_cursor = FlowCursor(session_id="attach-p", step_id=None)
        ...
        cursor = self.fastapi_app.state.flow_cursor
        # Cursor moved to the findings item.
        self.assertEqual(cursor.session_id, "findings-b")

        # Pinned attach item still present in the full queue.
        sessions = self.manager.list_sessions()
        queue = build_attention_queue(sessions, datetime.now(UTC))
        session_ids = [item.session_id for item in queue]
        self.assertIn("attach-p", session_ids)
...
class TestFlowSelect(_FlowTestBase):
    """Tests for POST /command-center/flow/select."""

    def test_select_sets_cursor_and_renders_active(self) -> None:
        """Selecting a specific session_id + step_id sets the cursor and renders body for it."""
        from zing_ai.server.flow import FlowCursor

        step_id_1 = self._make_findings_session("sel-1", "Select 1")
        step_id_2 = self._make_findings_session("sel-2", "Select 2")

        # Start at first item.
        self.fastapi_app.state.flow_cursor = FlowCursor(session_id="sel-1", step_id=step_id_1)

        # Select the second item explicitly.
        with self.client.stream(
            "POST",
            "/command-center/flow/select",
            json={"session_id": "sel-2", "step_id": step_id_2},
        ) as resp:
            self.assertEqual(resp.status_code, 200)
            events = _parse_sse(resp)

        # Cursor must point at the second item.
        cursor = self.fastapi_app.state.flow_cursor
        self.assertEqual(cursor.session_id, "sel-2")
        self.assertEqual(cursor.step_id, step_id_2)

        # SSE response must contain the findings body fragment for the second item.
        combined = "\n".join(events)
        self.assertIn("#flow-body", combined)
        # The findings body renders "flow-body-findings" class.
        self.assertIn("flow-body-findings", combined)

    def test_select_with_empty_step_id_treats_as_none(self) -> None:
        """Selecting with an empty string step_id normalises to None (attach-style cursor)."""
        self._make_attach_session("sel-attach", pinned=True)

        with self.client.stream(
            "POST",
            "/command-center/flow/select",
            json={"session_id": "sel-attach", "step_id": ""},
        ) as resp:
            self.assertEqual(resp.status_code, 200)
