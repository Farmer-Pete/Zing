"""Tests for get_live_tmux_sessions() in command_center.py."""
...
class TestGetLiveTmuxSessions(unittest.TestCase):
    """Tests for get_live_tmux_sessions()."""

    def test_get_live_tmux_sessions_parses_output(self) -> None:
        """Parses multi-line tmux output into a set of session names."""
        mock_result = MagicMock()
        mock_result.returncode = 0
        mock_result.stdout = "session1\nsession2\nsession3\n"
        with patch("subprocess.run", return_value=mock_result) as mock_run:
            result = get_live_tmux_sessions()
    ...
    def test_get_live_tmux_sessions_tmux_not_running(self) -> None:
    ...
    def test_get_live_tmux_sessions_tmux_not_installed(self) -> None:
