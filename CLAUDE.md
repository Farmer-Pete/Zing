...
- Before committing, run `make fmt lint` and `go build ./...`. The lefthook pre-commit hook runs the same checks, so a commit that fails the hook fails for the same reason. `make ci` runs everything CI's checks job runs; the secrets job additionally runs a full-history gitleaks scan.
